...
def print_table_values(table_name:str):
    ...
    try:
        ...
        # Print the rows
        print(f"{table_name} Table ({len(rows)} rows):")
        for row in rows:
            print(row)
...
def verify_login_into_website_login(user_name:str,password:str) -> bool:
    ...
    command = """
        SELECT * FROM website_login where user_name = %s AND hashed_password = %s
        """
    ...
    if len(result) == 1 and result != ERROR_WITH_DB_EXEC_COMMAND_CODE:
        return True
    return False
def get_all_emails_of_users_from_host_name(hostname:str) -> list:
    command = """
        SELECT email FROM website_login where host_name = %s 
        """
    args = (hostname,)
    result = exec_command(command,args)
